...
export function redwoodjs_vsc_lsp_treeview_setup(opts: {
  ...
  const treeView = vscode.window.createTreeView(redwoodjs_treeview_id, {
    treeDataProvider: {
      ...
      async getTreeItem(id) {
        ...
        if (command) command = processCommand(command)
        const item2 = { ...rest, iconPath, resourceUri, command }
        if (item2.menu) item2.contextValue = contextValue(item2.menu.kind)
